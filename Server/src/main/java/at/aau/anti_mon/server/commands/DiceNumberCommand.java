...
public class DiceNumberCommand implements Command{
    ...
    public void execute(WebSocketSession session, JsonDataDTO jsonData) throws CanNotExecuteJsonCommandException {
        if (jsonData.getData() == null || jsonData.getData().get("username") == null || jsonData.getData().get("dicenumber") == null || jsonData.getData().get("pin") == null) {
        ...
        Integer dicenumber = Integer.parseInt(jsonData.getData().get("dicenumber"));
        Integer pin = Integer.parseInt(jsonData.getData().get("pin"));

        eventPublisher.publishEvent(new DiceNumberEvent(session,username, dicenumber,pin));
    }
}
